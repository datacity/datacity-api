...
function serializeJSON(currentKey, currentObject, result, isArray) {
    for (var key in currentObject) {
        if ((typeof currentObject[key] === "string" || typeof currentObject[key] === "number") && isArray === false)
        ...
        if (currentObject[key] instanceof Array) 
            serializeJSON(key, currentObject[key], result, true);
        else if (currentObject[key] instanceof Object && isArray === false)
            serializeJSON(key, currentObject[key], result, false);
        else if (currentObject[key] instanceof Object && isArray === true)
            serializeJSON(currentKey, currentObject[key], result, true);
    }
}
          ...
          if (json[keys[index]] instanceof Array) {
             if (json[keys[index]][0] && json[keys[index]][0] instanceof Object)
               for (var obj in json[keys[index]]) {
                   result = {};
                   serializeJSON("", json[keys[index]][obj], result, false);
                   main.push(result);
               }
               break;
          }
...
parserJSON().parse(__dirname+"/../test/files/input/JSON_test.json", function(data) {
    var stream = fs.createWriteStream(__dirname+"/../test/files/output/JSON_testParse.json");
    stream.write(JSON.stringify(data));
});
